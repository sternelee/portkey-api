import { AppContext, tableSchemas, JSFilter, ApiConfig } from './index';
...
export const hasUser = async (ctx: AppContext) => {
  ...
  const result = await getRecords(ctx, 'custom', {}, fn);
...
export async function getApiAccessControlResult(
  ...
  filterAccessControl:
    ...
    | ((...args: any[]) => boolean | Promise<boolean>)
    | JSFilter
    | ((...args: any[]) => JSFilter | Promise<JSFilter>),
  itemAccessControl: boolean | ((...args: any[]) => boolean | Promise<boolean>),
  ctx: AppContext,
  ...args: any[]
) {
  let authorized: boolean | JSFilter = await getAccessControlResult(
    operationAccessControl,
    ctx,
    args[0],
    args[2]
...
async function getAccessControlResult(
  ...
  let authorized: boolean | JSFilter = true;
...
export async function getItemAccessControlResult(
  ...
  if (typeof itemAccessControl === 'boolean') {
    ...
    const doc = await getRecords(ctx, table, { id });
